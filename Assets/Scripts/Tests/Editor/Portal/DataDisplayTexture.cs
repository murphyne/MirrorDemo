...
namespace Tests.Editor.Portal
{
    public class DataDisplayTexture
    {
        ...
        private static readonly Color ColorP = new Color(0.90f, 0.40f, 0.90f);
        private static readonly Color ColorQ = new Color(0.90f, 0.40f, 0.90f);
        ...
        public void Render()
        {
            ...
            var pPosCWorld = _data.pPos;
            var pDirFWorld = _data.pDir;
            var qPosCWorld = _data.qPos;
            var qDirFWorld = _data.qDir;
            var pDirLWorld = pPosCWorld + Vector3.Cross(pDirFWorld - pPosCWorld, _data.pUp - pPosCWorld);
            var pDirRWorld = pPosCWorld + Vector3.Cross(_data.pUp - pPosCWorld, pDirFWorld - pPosCWorld);
            var qDirLWorld = qPosCWorld + Vector3.Cross(qDirFWorld - qPosCWorld, _data.qUp - qPosCWorld);
            var qDirRWorld = qPosCWorld + Vector3.Cross(_data.qUp - qPosCWorld, qDirFWorld - qPosCWorld);
            ...
            var pPosCXPixel = (int)ToXPixel(pPosCWorld.x);
            var pPosCZPixel = (int)ToZPixel(pPosCWorld.z);
            var pDirFXPixel = (int)ToXPixel(pDirFWorld.x);
            var pDirFZPixel = (int)ToZPixel(pDirFWorld.z);
            var pDirLXPixel = (int)ToXPixel(pDirLWorld.x);
            var pDirLZPixel = (int)ToZPixel(pDirLWorld.z);
            var pDirRXPixel = (int)ToXPixel(pDirRWorld.x);
            var pDirRZPixel = (int)ToZPixel(pDirRWorld.z);
            var qPosCXPixel = (int)ToXPixel(qPosCWorld.x);
            var qPosCZPixel = (int)ToZPixel(qPosCWorld.z);
            var qDirFXPixel = (int)ToXPixel(qDirFWorld.x);
            var qDirFZPixel = (int)ToZPixel(qDirFWorld.z);
            var qDirLXPixel = (int)ToXPixel(qDirLWorld.x);
            var qDirLZPixel = (int)ToZPixel(qDirLWorld.z);
            var qDirRXPixel = (int)ToXPixel(qDirRWorld.x);
            var qDirRZPixel = (int)ToZPixel(qDirRWorld.z);

            // Draw points.
            DrawSymbol(texture, aPosXPixel, aPosZPixel, 'A', ColorA);
            DrawSymbol(texture, bPosXPixel, bPosZPixel, 'B', ColorB);
            DrawSymbol(texture, pPosCXPixel, pPosCZPixel, '+', ColorP);
            DrawSymbol(texture, qPosCXPixel, qPosCZPixel, '+', ColorQ);
            ...
            DrawLine(texture, pDirLXPixel, pDirLZPixel, pDirRXPixel, pDirRZPixel, ColorP);
            DrawLine(texture, pPosCXPixel, pPosCZPixel, pDirFXPixel, pDirFZPixel, ColorP);
            DrawLine(texture, qDirLXPixel, qDirLZPixel, qDirRXPixel, qDirRZPixel, ColorQ);
            DrawLine(texture, qPosCXPixel, qPosCZPixel, qDirFXPixel, qDirFZPixel, ColorQ);
        ...
        {
            const string dirPathRel = "Assets/Scripts/Tests/PortalData/Textures";
